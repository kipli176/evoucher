...
class Home extends BaseController
{
    ...
    public function nopol()
    {
        ...
        $response = formCari($sesi, $cookie, $nopol);
        $nopole = json_decode($response, true);
        // $response = $client->request('GET', '/ceknopol.php', ['verify' => false, 'query' => ['nopol' => $nopol]]);
        ...
        if (getNopol('data', $nopol) != '') {
            $data['errors'] = ['Maaf nopol sudah pernah menggunakan eVoucher'];
            $data['id_merchant'] = $this->request->getPost('id_merchant');

            return view('verif', $data);
        } elseif ($nopole['pemilik'] == '') {
            $data['errors'] = ['Maaf nopol kendaraan anda belum terdaftar dalam sistem atau belum tersedia'];
            $data['id_merchant'] = $this->request->getPost('id_merchant');
